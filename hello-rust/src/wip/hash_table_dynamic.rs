...
impl HashTable {
    ...
    fn integer_hash32(mut h: u32) -> u32 {
    	h ^= h >> 16;
    ...
    pub fn set_item(&self, key:u64, value:u64) {

	//0 reserved for 'empty' value
	assert!(key != 0);
	assert!(value != 0);

	let mut idx = HashTable::integer_hash64(key);
	loop {

	    //scale to size of array
	    idx &= self.m_array_size - 1;

	    let result_key = self.m_entries[idx.try_into().unwrap()].key.compare_and_swap(0, key, Ordering::Relaxed);

	    if result_key == 0 || result_key == key {
		self.m_entries[idx.try_into().unwrap()].value.store(value, Ordering::Relaxed);
    ...
    pub fn get_item(&self, key:u64) -> u64 {
	...
	loop {
	    ...
	    let probed_key = self.m_entries[idx.try_into().unwrap()].key.load(Ordering::Relaxed);
	    if probed_key == key {
		return self.m_entries[idx.try_into().unwrap()].value.load(Ordering::Relaxed);
	    }
	    if probed_key == 0 {
		return 0
	    }
	...
	for i in 0..self.m_array_size {
	    print!("{}, ", self.m_entries[i.try_into().unwrap()].value.load(Ordering::Relaxed))
	}
    }
    //-------------END DEBUG-------------------
